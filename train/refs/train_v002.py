...
from dataset.mx_data_iter_v2 import RecDataIterV1
import time
...
def train_v1(opts):
    ...
    ## network
    net = resnet18_v2(classes=opts.num_classes)
    ...
    ## optimizer
    lr_scheduler = mx.lr_scheduler.FactorScheduler(step=5, factor=0.1, base_lr=opts.learning_rate)
        ...
        for idx_step, batch_data in enumerate(iter(train_iter)):
            ...
            with mx.autograd.record():
                for datai, targeti in zip(datas, targets):
                    ...
                    # print('[===] targeti min-max: {}  {}  {}'.format(targeti.shape, mx.nd.max(targeti[0, :]).asnumpy()[0], mx.nd.min(targeti[0, :]).asnumpy()[0]))
                    # print('[===] predicts min-max: {}  {}  {}'.format(predicts.shape, mx.nd.max(predicts[0, :]).asnumpy()[0], mx.nd.min(predicts[0, :]).asnumpy()[0]))
                    ...
                    ## update metrics
                    loss_metric.update(mse_loss.shape[0], mse_loss.asnumpy().sum())
            ...
            if (global_step + 1) % opts.log_interval == 0:
                logger.info('[{}/{}][{}/{}] [loss: {:.10f} / {:.10f}]  [{} samples/s]  [lr: {:.10f}]'.format(
                    epoch,
                    opts.max_epoch-opts.min_epoch,
                    idx_step,
                    int(train_iter.max_index/opts.batch_size),
                    loss_metric.get_avg()[0],
                    loss_metric.get_avg()[1],
                    samples_per_second,
