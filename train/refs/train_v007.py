...
from config.config_v007 import Config
# import log.logger.Logger as logger
import device_control.device_info as DeviceInfo
from dataset.mx_data_iter_v006 import RecDataIterV1
import time
...
from network.mobilenet_v004 import get_mobilenet_v2
...
def train_v1(opts):
    ...
    ## optimizer
    lr_scheduler = mx.lr_scheduler.FactorScheduler(step=50, factor=0.1, base_lr=opts.learning_rate, stop_factor_lr=1e-4)
        ...
        for idx_step, batch_data in enumerate(iter(train_iter)):
            ...
            with mx.autograd.record():
                for datai, targeti in zip(datas, targets):
                    ...
                    mse_loss1 = loss_funcs[0](targeti[:, 0:68], predicts[:, 0:68])
                    mse_loss2 = loss_funcs[0](targeti[:, 68:], predicts[:, 68:])
            ...
            if (global_step + 1) % opts.log_interval == 0:
                logger.info('[{}/{}][{}/{}] [loss: {:.6f} / {:.6f}  {:.6f} / {:.6f}  {:.6f} / {:.6f}]  [{} samples/s]  [lr: {:.10f}]'.format(
                    epoch,
