...
mx.gluon.model_zoo.vision.mobilenet_v2_1_0()
...
class Lemon(mx.gluon.nn.HybridBlock):
    ...
    def hybrid_forward(self, F, x):
        ...
        print(f'[Lemon] {self.name} att1: {att1.shape if not isinstance(att1, mx.symbol.Symbol) else att1}')
        att2 = self.attention2(x)
        print(f'[Lemon] {self.name} att2: {att2.shape if not isinstance(att2, mx.symbol.Symbol) else att2}')
...
class Output(mx.gluon.nn.HybridBlock):
    ...
    def hybrid_forward(self, F, x):
        ...
        print(f'[Output] landmarks: {landmarks.shape if not isinstance(landmarks, mx.symbol.Symbol) else landmarks}')
        if self.keypoints:
...
if __name__ == '__main__':
    ...
    if keypoints:
        ...
        print(f'[main] y: {y.shape}')

    net.export(r'../checkpoints/FM/v2/ok/init', 0)
