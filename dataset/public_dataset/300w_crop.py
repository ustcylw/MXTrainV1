...
def crop_size(root_dir, shape=(128,128)):
    video_path = os.path.join(root_dir, 'images')
    video_dir_names = os.listdir(video_path)
    skip_list = []
    crop_path = os.path.join(root_dir, 'crop')
    if not os.path.exists(crop_path):
        os.makedirs(crop_path)
        ...
        os.makedirs(crop_path)
    for idx1, video_dir_name in enumerate(video_dir_names):
        image_path = os.path.join(video_path, video_dir_name)
        if not os.path.isdir(image_path):
            continue
        image_names = os.listdir(image_path)
        process_count = 0
        video_crop_path = os.path.join(crop_path, video_dir_name)
        if not os.path.exists(video_crop_path):
            os.makedirs(video_crop_path)
        for idx2, image_name in enumerate(image_names):
            if not image_name.endswith('.png'):
                continue
            image_file = os.path.join(image_path, image_name)
            points_file = ''+image_file
            points_file = points_file.replace('png', 'pts')
            point_lines = None
            with open(points_file, 'r') as f:
                point_lines = f.readlines()[3:71]
            image = cv2.imread(image_file)
            ...
            if False:
                ...
                for i in range(68):
                    p = points_[i, :]
                    print(p)
                    cv2.circle(image_, (int(round(p[0])), int(round(p[1]))), 1, (0, 0, 255))
            ...
            image_save_file = os.path.join(video_crop_path, image_name)
            cv2.imwrite(image_save_file, image_)
            points_save_file = os.path.join(video_crop_path, image_name.replace('jpg', 'pts'))
            with open(points_save_file, 'w+') as f:
                f.write(points_str)
                f.flush()
            process_count += 1
    ...
    for skip in skip_list:
        skip_str += skip
        skip_str += '\n'
    with open('./300w_skip_list.txt', 'w+') as f:
        f.write(skip_str)
        f.flush()
...
if __name__ == '__main__':
    ...
    root_dir = '/media/bill/cc18c1ef-ab04-47e4-ad1f-f431ab2785a3/datasets/300vw'
